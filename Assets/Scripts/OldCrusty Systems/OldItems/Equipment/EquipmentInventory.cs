...
{
    Dictionary<EquipmentSlotKey, InventorySlot<Unit, EquipmentItem>> m_equipmentSlots;
    ...
    public EquipmentInventory(Unit owner) : base(owner)
    {
        ...
        m_equipmentSlots = new Dictionary<EquipmentSlotKey, InventorySlot<Unit, EquipmentItem>>(m_slotCount);
        for(int i = 0; i < m_slotCount; i++)
        {
            EquipmentSlotKey key = (EquipmentSlotKey)enumValues.GetValue(i);
            m_equipmentSlots.Add(key, new InventorySlot<Unit, EquipmentItem>(this, i));
        }
    }
    ...
    {
        BaseItem alreadyEquippedItem = m_equipmentSlots[slotKey].GetItemType();
    ...
    public InventorySlot<Unit, EquipmentItem> GetEquipSlot(EquipmentSlotKey slotKey)
    {
        return m_equipmentSlots[slotKey];
    }

    public float GetCoreStatModifierValue(CoreStatKey coreStatKey)
    {
        float value = 0.0f;
        foreach(var equipSlot in m_equipmentSlots)
        {
            EquipmentItem equipment = equipSlot.Value.GetItemType();
            if (equipment != null)
            {
                value += equipment.GetCoreStatModifier(coreStatKey);
            }
        }
        return value;
    }
